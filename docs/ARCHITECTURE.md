...
> **Internal engineering reference (moved from the root README). This file is the canonical overview of the project and is written against the code, not against a plan.** Every number, gate name, endpoint count and limit below was read out of the source at the time of writing. Where something is built but not wired up, [§12](#12-what-is-wired-and-what-is-a-seam) says so by name — a README that only lists what works is a sales page, and the first person it misleads is the next contributor.
